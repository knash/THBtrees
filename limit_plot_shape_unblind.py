...
if __name__ == "__main__":
	...
	WprimeConvo*=sigbrdict[str(int(data[0]))][mindex]
		...
		y_mclimit.append( data[1]*rt_xsec*WprimeConvo )

		y_mclimitlow95.append( data[2]*rt_xsec*WprimeConvo )
		y_mclimitup95.append( data[3]*rt_xsec*WprimeConvo )
		y_mclimitlow68.append( data[4]*rt_xsec*WprimeConvo )
		y_mclimitup68.append( data[5]*rt_xsec*WprimeConvo )
	else:
		print "noscale"
    ...
    if (not options.noTheory) :
		...
		y_limit.append( data[1]*rt_xsec*WprimeConvo )
    ...
    if True : 
	    ...
	    g_limit.GetXaxis().SetRangeUser(1.5, 4.0)
	    if logScale:
		g_limit.SetMinimum(1.0e-4) #0.005
		g_limit.SetMaximum(1000.) #10000
	    else:
    ...
    TPT.Draw()		
    cv.RedrawAxis()
    print "name is limits_theta_unblind_"+postpend+options.region
    gPad.Update()
    cv.SaveAs("plots/limits_theta_unblind_"+postpend+options.region+".pdf")
    cv.SaveAs("plots/limits_theta_unblind_"+postpend+options.region+".gif")
    cv.SaveAs("plots/limits_theta_unblind_"+postpend+options.region+".png")
    cv.SaveAs("plots/limits_theta_unblind_"+postpend+options.region+".root")
    rootfile = TFile( "limits_outputlim_theta_unblind_"+postpend+options.region+".root", "recreate" )
    rootfile.cd()
    g_limit.Write("obs")
    g_mclimit.Write("exp")
    graphWP.Write("WP")
    graphWPup.Write("WPup")
    graphWPdown.Write("WPdown")
    rootfile.Write()
    rootfile.Close()
    if (not options.noTheory) :
