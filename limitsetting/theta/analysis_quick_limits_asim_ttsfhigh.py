...
def limits_allhad(model, step = 0):
   ...
   canvnuisanceplot = makenuisanceplot(parlist,parVals['wp1500'],"bkg1500_ttsfhigh")
   ...
   myopts = Options()
   myopts.set('minimizer', 'strategy', 'robust')
   parVals = mle(model, input = 'toys-asimov:1', n=1,with_error=True, with_covariance=False, signal_process_groups=None, options=myopts)
   print "SIGS"
   print parVals
   for pv in parVals:
	for pv1 in parVals[pv]:
   ...
   canvnuisanceplot = makenuisanceplot(parlist,parVals['wp1500'],"sig1500_ttsfhigh")
   canvnuisanceplot = makenuisanceplot(parlist,parVals['wp2000'],"sig2000_ttsfhigh")
