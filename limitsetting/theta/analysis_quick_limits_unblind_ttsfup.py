...
def build_allhad_model():
	...
	print p
    	...
    	model.add_asymmetric_lognormal_uncertainty('topsf',math.log(1.1),math.log(1.1), p)
       	if p=='ttbar':
...
def limits_allhad(model, step = 0):
   ...
   #print exp
   exp.write_txt('unblindedttsfup_exp'+fnamestr.replace('.root','')+'.txt')
   obs.write_txt('unblindedttsfup_obs'+fnamestr.replace('.root','')+'.txt')
   ...
   canvnuisanceplot = makenuisanceplot(parlist,parVals[''],"bkgttsfup_data")
