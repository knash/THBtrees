...
def limits_allhad(model, step = 0):
   ...
   print exp
   exp.write_txt('test_ttsfdown_exp'+fnamestr.replace('.root','')+'.txt')


   myopts = Options()
   myopts.set('minimizer', 'strategy', 'robust')
   parVals = mle(model, input = 'toys:0', n=1, with_error=True, with_covariance=False)
   for pv in parVals:
	print pv
	print parVals[pv]
   ...
   canvnuisanceplot = makenuisanceplot(parlist,parVals['wp2000'])
