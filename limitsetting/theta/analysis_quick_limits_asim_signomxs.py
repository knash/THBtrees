...
def limits_allhad(model, step = 0):
   ...
   canvnuisanceplot = makenuisanceplot(parlist,parVals['wp1500'],"sig1500_std_signomxs")


   signal_process_groups1 = {'wp2000': ['wp2000']}

   myopts = Options()
   myopts.set('minimizer', 'minuit_tolerance_factor', '2.0')
   parVals = mle(model, input = 'toys-asimov:1', n=1,with_error=True, with_covariance=False, signal_process_groups=signal_process_groups1, options=myopts)
   print "SIGS"
   print parVals
   for pv in parVals:
	for pv1 in parVals[pv]:
		print pv1
		print parVals[pv][pv1]
   canvnuisanceplot = makenuisanceplot(parlist,parVals['wp2000'],"sig2000_std_tol2_signomxs")
