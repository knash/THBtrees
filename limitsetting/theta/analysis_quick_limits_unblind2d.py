...
def histogram_filter(hname):
    if 'wp1500' in hname: return False
    if 'wp2500' in hname: return False
    if 'wp3000' in hname: return False
    if 'wp3500' in hname: return False
    if 'wp4000' in hname: return False
...
def limits_allhad(model, step = 0):
   ...
   #print exp
   exp.write_txt('unblinded2d_exp'+fnamestr.replace('.root','')+'.txt')
   obs.write_txt('unblinded2d_obs'+fnamestr.replace('.root','')+'.txt')
