...
def build_allhad_model():
	...
	print p
    	...
    	#model.add_asymmetric_lognormal_uncertainty('topsf',math.log(1.06),math.log(1.2), p)
    	#model.add_lognormal_uncertainty('topsf', math.log(1.1), p)
       	if p=='ttbar':
...
def limits_allhad(model, step = 0):
   ...
   #parVals = mle(model, input = 'toys-asimov:0', n=1,with_error=True, with_covariance=False, options=myopts)
   #parlist = ['__nll','beta_signal']
   #canvnuisanceplot = makenuisanceplot(parlist,parVals['wp1500'],"bkg1500_std")
   ...
   signal_process_groups1 = {'wp1500': ['wp1500']}

   myopts = Options()
   myopts.set('minimizer', 'strategy', 'robust')
   parVals = mle(model, input = 'toys-asimov:1', n=1,with_error=True, with_covariance=False, signal_process_groups=signal_process_groups1, options=myopts)
   print "SIGS"
   print parVals
   for pv in parVals:
	for pv1 in parVals[pv]:
		...
		print parVals[pv][pv1]
   parlist = ['__nll','beta_signal']
   canvnuisanceplot = makenuisanceplot(parlist,parVals['wp1500'],"sig1500_std_1pb")

   parVals = mle(model, input = 'toys-asimov:0.1', n=1,with_error=True, with_covariance=False, signal_process_groups=signal_process_groups1, options=myopts)
   ...
   canvnuisanceplot = makenuisanceplot(parlist,parVals['wp1500'],"sig1500_std_0p1pb")


   parVals = mle(model, input = 'toys-asimov:0.01', n=1,with_error=True, with_covariance=False, signal_process_groups=signal_process_groups1, options=myopts)
   ...
   canvnuisanceplot = makenuisanceplot(parlist,parVals['wp1500'],"sig1500_std_0p01pb")




   parVals = mle(model, input = 'toys-asimov:2.0', n=1,with_error=True, with_covariance=False, signal_process_groups=signal_process_groups1, options=myopts)
   ...
   canvnuisanceplot = makenuisanceplot(parlist,parVals['wp1500'],"sig1500_std_2pb")
