...
for cregion in regions:
	...
	for coup in ['right']:
		...
		for sigpoint in sigpoints:
			...
			efftable[sigpoint[0]+sigpoint[1]] = nointentries/nototentries
			conttable[sigpoint[0]+sigpoint[1]] =  sigfiles[-1][0].Get("Mthb_h1t1b1").Integral()+sigfiles[-1][1].Get("Mthb_h1t1b1").Integral()
	   	...
	   	for ch in chans:
			...
			pdffraction = {}
			for sigpoint in sigpoints:
				uncuptable[sigpoint[0]+sigpoint[1]] = 0
				uncdowntable[sigpoint[0]+sigpoint[1]] = 0
				for syststring in syststrings: 
					...
					output.cd()
					print "writing ",reconamesys
					Deviation = allsyssighists[-1].Integral()-conttable[sigpoint[0]+sigpoint[1]]
					print "Dev,",Deviation
					if Deviation>0:
						uncuptable[sigpoint[0]+sigpoint[1]] += Deviation*Deviation
						print "up",sqrt(uncuptable[sigpoint[0]+sigpoint[1]])
					else:
						uncdowntable[sigpoint[0]+sigpoint[1]] += Deviation*Deviation
						print "down",sqrt(uncdowntable[sigpoint[0]+sigpoint[1]])
				...
				for i in range(0,len(D)):
					...
					if True:
						...
						if reconame=="mthb_"+ch+"__wp"+sigpoint[0]:
							...
							print "nomtest",conttable[sigpoint[0]+sigpoint[1]],nomhistsig.Integral()
	...
	for eff in efftable:
		...
		uncuptot=sqrt(uncuptable[eff])/conttable[eff]
		uncdowntot=sqrt(uncdowntable[eff])/conttable[eff]
		#print "upspre",uncuptot		
		#print "downspre",uncdowntot
		uncuptot += sqrt(0.025*0.025+0.1*0.1+0.05*0.05)
		uncdowntot += sqrt(0.025*0.025+0.04*0.04+0.05*0.05)
		
		print eff,WPF.strfSF(100.0*efftable[eff],2),"$\\pm$",WPF.strfSF(0.5*(uncuptot+uncdowntot)*(100.0*efftable[eff]),2)
		#print "+",uncuptot,"-",uncdowntot
